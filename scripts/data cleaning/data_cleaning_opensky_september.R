...
open_sky_september_2019   <- readr::read_csv("data/raw_data/opensky_september_2019.csv.gz")
open_sky_september_2020   <- readr::read_csv("data/raw_data/opensky_september_2020.csv.gz")
...
flight_data_clean_september_2019   <- open_sky_september_2019 %>% clean_flight_data()
flight_data_clean_september_2020   <- open_sky_september_2020 %>% clean_flight_data()

#--- summarising 2019 data
total_flights_september_2019 <- flight_data_clean_september_2019 %>%
...
total_flights_september_2020 <- flight_data_clean_september_2020 %>%
  dplyr::filter(origin_country != destination_country) %>%
  dplyr::group_by(origin_country, destination_country) %>%
...
total_flights_september_2019_2020 <- total_flights_september_2019 %>% 
  dplyr::ungroup() %>%
  dplyr::left_join(total_flights_september_2020) %>%
...
#--- in the absence of an estimate. Comes out to 0.521. Very consistent with the IATA 
#--- estimate
...
write.csv(total_flights_september_2019_2020, "data/flight_reduction_scaling_factors_september.csv")
